...
s = cell(L,1); % intermediate values
t = cell(L,1); % intermediate values
...
for p = 1:n
  ...
  d{L} = 2*(t{L} - y(p)) * dactfun(t{L});
  for l = L-1:-1:1
    d{l} = dactfun(t{l}) .* (W{l+1}*d{l+1});
  end

  % compute gradient (accumulate for each data point)
  temp = x(:,p);
  for l = 1:L
    dW{l} = dW{l} + temp*d{l}';  % outer product
    db{l} = db{l} + d{l};
    temp = t{l};
  end
